...
if __name__ == "__main__":
    # 处理并保存特征数据
    feature_df = process_emg_data(r'E:\fight_for_py\CyBergearExoskeleton\DIYbyCybergear-main\motion_control\Pyserial-Demo-master\motor_intention.csv')
    ...
    pd.concat([pd.DataFrame(scaler.transform(X), columns=X.columns), y], axis=1) \
      .to_csv(r'E:\fight_for_py\CyBergearExoskeleton\DIYbyCybergear-main\motion_control\Pyserial-Demo-master\motor_intention_features.csv', index=False)