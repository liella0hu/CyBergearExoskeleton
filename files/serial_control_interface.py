...
class SerialControllerInterface:
    ...
    def enable_motor(self):
        ...
        if length == 3 and len(data[length - 2].hex()) >= 20:
            # print("接收到read_param的数据帧:", data[length-1].hex())
            print("if length == 3 cybergear, you")
            # return self._parse_received_msg(data[length - 1], "ba")
    ...
    def disable_motor(self):
        ...
        data = self.serial.readlines() 
        length = len(data)
        print("disable_motor original data", data)
        if length == 2 and len(data[length - 1].hex()) >= 20:
            # print("接收到read_param的数据帧:", data[length-1].hex())
            return self._parse_received_msg(data[length - 1], "ba")
        if length == 3 and len(data[length - 2].hex()) >= 20:
            # print("接收到read_param的数据帧:", data[length-1].hex())
            return self._parse_received_msg(data[length - 2], "ba")
    ...
    def read_single_parameter(self, parameter_name):
        ...
        if parameter_name in self.PARAMETERS:
            ...
            if length == 3 and  len(data[length-2].hex()) >= 20:
                # print("接收到read_param的数据帧:", data[length-1].hex())
                return self._parse_received_msg(data[length-2], format)
        else:
...
if __name__ == "__main__":
    motor1 = SerialControllerInterface(1, port="COM9")
    motor2 = SerialControllerInterface(2, port="COM9")
    motor1.enable_motor()
    motor2.enable_motor()
    ...
    # time.sleep(1.5)
    motor1.set_motor_position_control(2.0, 50.0)
    motor2.set_motor_position_control(2.0, 70.0)
    #
    time.sleep(50)
    # motor2.set_motor_0position()
